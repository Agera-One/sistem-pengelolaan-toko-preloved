        ...
        [
            ...
            'children' => [
                [
                    ...
                    'active' => 'pembelian.pesanan.index',
                ],
                [
                    'label'  => 'Pembayaran',
        ...
        [
            ...
            'children' => [
                [
                    ...
                    'active' => 'penjualan.index',
                ],
                [
                    'label'  => 'Retur Penjualan',
                    'route'  => 'penjualan.retur.index',
                    'active' => 'penjualan.retur.*',
                ],
                [
                    'label'  => 'Pembayaran Penjualan',
                    'route'  => 'penjualan.pembayaran.index',
                    'active' => 'penjualan.pembayaran.*',
                ],
            ],
        ],
        [
    ...
    <nav aria-label="Menu utama" class="flex-1 overflow-y-auto px-4 py-5">
        <ul class="space-y-1">
            @foreach ($menus as $i => $menu)
                @if (($menu['type'] ?? 'link') === 'group')
                    @php
                        $isChildActive = collect($menu['children'])->contains(
                            fn ($child) => request()->routeIs($child['active'])
                        );
                        $submenuId = 'submenu-' . $i;
                    @endphp
                    <li>
                        <button type="button"
                                ...
                                @class([
                                    'flex min-h-12 w-full items-center gap-3 rounded-lg px-3 py-2 text-lg transition duration-200 ease-in-out focus:outline-none focus-visible:ring-4 focus-visible:ring-brand/40',
                                    'bg-brand/10 font-semibold text-brand-hover' => $isChildActive,
                                    'font-medium text-stone-700 hover:bg-stone-200/60 hover:text-stone-900' => ! $isChildActive,
                                ])>
                            <svg @class(['h-6 w-6 shrink-0', 'text-brand' => $isChildActive, 'text-stone-600' => ! $isChildActive])
                                 xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                 stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $menu['icon'] }}"/>
                            </svg>
                            <span class="flex-1 text-left">{{ $menu['label'] }}</span>
                            <svg data-sidebar-chevron
                                 @class([
                                     'h-5 w-5 shrink-0 transition-transform duration-200 ease-in-out',
                                     'rotate-180' => $isChildActive,
                                     'text-brand' => $isChildActive,
                                     'text-stone-500' => ! $isChildActive,
                                 ])
                                 xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                 stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                        ...
                        <ul id="{{ $submenuId }}"
                            @class(['mt-1 space-y-1 pl-4', 'hidden' => ! $isChildActive])>
                            @foreach ($menu['children'] as $child)
                                ...
                                <li>
                                    <a href="{{ $href }}"
                                       ...
                                       @class([
                                           'flex min-h-11 items-center gap-3 rounded-lg border-l-2 py-2 pl-4 pr-3 text-base transition duration-200 ease-in-out focus:outline-none focus-visible:ring-4 focus-visible:ring-brand/40',
                                           'border-brand bg-brand/10 font-semibold text-brand-hover' => $isActive,
                                           'border-stone-200 font-medium text-stone-600 hover:border-stone-400 hover:bg-stone-200/60 hover:text-stone-900' => ! $isActive,
                                       ])>
